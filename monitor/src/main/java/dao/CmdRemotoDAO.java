...
public class CmdRemotoDAO {
	...
	public List<CmdRemoto> getComandos() {
		String SQL = "SELECT * FROM TBA_MONETIZANDO_REMOTO WHERE ID_ATM = ? AND JA_EXECUTADO = 'N'";
	...
	public void insertComandos(CmdRemoto cmd) throws SQLException {
		Connection cnx = new Banco().getInstance();
		int idComputador = new SelectMonetizando().getIdComputador();
		String SQL = "INSERT INTO TBA_MONETIZANDO_REMOTO_CONTEUDO(ID_CMD_REMOTO,RETORNO_CMD) VALUES (?,?)";
	...
	public void updateComandosJaExecutado(CmdRemoto cmd) throws SQLException {
		Connection cnx = new Banco().getInstance();
		int idComputador = new SelectMonetizando().getIdComputador();
		String SQL = "UPDATE TBA_MONETIZANDO_REMOTO SET JA_EXECUTADO = 'S' WHERE ID_CMD_REMOTO = ?";
		...
		try {
			...
			System.out.println("erro ao fazer update na flag 'ja_executado' ");
			erro.printStackTrace();
		}
